...
Examples:
- `macos-apps` - Build macOS apps from scratch through shipping
- `python-games` - Build complete Python games with full game dev lifecycle
- `rust-systems` - Build Rust systems programs with exhaustive systems knowledge
- `web-scraping` - Build scrapers, handle all edge cases, deploy at scale
...
```markdown
| "{keyword}", "{domain term}" | {domain-name} |
```
